...
export const RealTimeStats: React.FC<RealTimeStatsProps> = ({ onClose }) => {
  ...
  const locations = [
    // Watergates
    {
      id: "bendung-katulampa",
      name: "Bendung Katulampa",
      type: "watergate" as const,
    },
    ...
    const interval = setInterval(() => {
      ...
      setLocationData((prevData) =>
        prevData.map((locationData) => {
          const newLevel =
            locationData.currentLevel + (Math.random() - 0.5) * 0.02;
  ...
  const watchLevels = locationData.filter(
    (l) => l.currentLevel >= 2.8 && l.currentLevel < 3.5
  ).length;
  const alertLevels = locationData.filter((l) => l.currentLevel >= 3.5).length;
  const avgLevel =
    locationData.length > 0
      ? locationData.reduce((sum, l) => sum + l.currentLevel, 0) /
        locationData.length
      : 0;
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h2 className="text-3xl font-bold">
                  Real-Time Monitoring Tinggi Air
                </h2>
                <p className="text-blue-100 mt-1 font-medium">
                  Live Sensor IoT Tinggi Air
                </p>
              </div>
            </div>
            <Button
        ...
        <div className="p-6 overflow-y-auto h-full">
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center space-x-2 mt-1">
                      <span
                        className={`inline-block w-2 h-2 rounded-full ${getStatusBgColor(
                          location.currentLevel
                        )} animate-pulse`}
                      ></span>
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        {location.locationType === "watergate"
                          ? "Pintu Air"
                          : "Daerah"}
                      </p>
                    </div>
                ...
                <ResponsiveContainer width="100%" height={100}>
                  <LineChart data={location.data}>
                    ...
                    <Line
                      ...
                      dataKey="waterLevel"
                      stroke={
                        location.currentLevel >= 3.5
                          ? "#ef4444"
                          : location.currentLevel >= 2.8
                          ? "#f97316"
                          : location.currentLevel >= 2.0
                          ? "#eab308"
                          : "#22c55e"
                      }
                      strokeWidth={2}
          ...
          <div className="mt-8 p-6 bg-gradient-to-r from-gray-50 to-blue-50 dark:from-gray-800 dark:to-gray-700 rounded-2xl border border-gray-200 dark:border-gray-600">
            <div className="flex items-center justify-center space-x-3 text-sm text-gray-600 dark:text-gray-400">
              ...
              <span>•</span>
              <span>
                {locationData.length} IoT water level sensors connected
              </span>
              <span>•</span>
